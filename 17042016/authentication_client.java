...
// TODO : Read file name & send file name
        ...
        // Variables
        int currentChunk = 0;
        int chunkSize = 64;                                     // Each chunck (for sending) is 64 bytes
        int startByte = 0;
        int endByte = 0;

        byte[] toEncryptBuffer = new byte[chunkSize];
        double chunkInDouble = Math.ceil(totalDataBytes.length / chunkSize);
        int numOfChunk = (int)chunkInDouble;
        endByte = endByte + chunkSize;


        outputStream = echoSocket.getOutputStream();
        FileInputStream fileInputStream_Send = new FileInputStream(fileName1);
        bufferedInputStream = new BufferedInputStream(fileInputStream_Send);

        while (currentChunk < numOfChunk) {
            bufferedInputStream.read(toEncryptBuffer, 0, toEncryptBuffer.length); //
            System.out.print(new String(toEncryptBuffer));
            byte[] encrypted = rsaCipher.doFinal(toEncryptBuffer);              // Do encryption

            // "Software caused connection abort: socket write error" : happens when the connection has been closed, either on the other side or locally
            ...
            currentChunk++;
            //System.out.println("Start" + startByte + " end" + endByte + " chunk" + currentChunk + " numChunk" + numOfChunk + " size" + chunkSize);
        }
